...
{ pkgs, inputs, user, ... }:
{
  ...
  # missing or the session won't start under the virtual GPU, fix it on the VM
  # side instead: Display → VMSVGA with 3D acceleration enabled (COSMIC needs a
  # working GL stack, it has no software-only fallback worth using).
